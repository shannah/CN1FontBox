...
public abstract class Collection
    extends Object
    implements TTFFont.Glyph.Collection
{

    private TTFFont.Glyph[] set;
    ...
    public TTFFont.Glyph getGlyph(char id){
    ...
    protected void add(TTFFont.Glyph glyph){
        int idx = glyph.getId();
        int cap = (idx+1);
        int len = (null != this.set)?(this.set.length):(0);
        if (cap > len){
            TTFFont.Glyph[] set = new TTFFont.Glyph[cap];
            if (0 != len)
    ...
    public static class Iterator
        extends Object
        implements java.util.Iterator<TTFFont.Glyph>
    {

        private final TTFFont.Glyph[] set;
        private final int length;
        private int index;


        public Iterator(TTFFont.Glyph[] set){
            super();
            this.set = set;
