...
public class TTFSubFont 
{
    ...
    public void addCharCode(int charCode) 
    {
        ...
        if (charCode == 0 || gid.intValue() != 0) 
        {
            characters.put(new Integer(charCode),gid);
            glyphIds.add(gid);
        }
    }
            ...
            throws IOException 
    {
        ...
        //LOG.debug("Writing table header [%s,%08x,%08x,%08x]",tag,checksum,offset,bytes.length));
    ...
    private static void writeLongDateTime(DataOutputStream dos, long dt) throws IOException 
    {
    ...
    private byte[] buildHheaTable() throws IOException 
    {
        ...
        writeUint16(dos,glyphIds.subSet(new Integer(0),new Integer(h.getNumberOfHMetrics())).size());
    ...
    private byte[] buildOS2Table() throws IOException 
    {
        ...
        writeUint16(dos,first.getKey().intValue());
        writeUint16(dos,characters.lastKey().intValue());
        /*
                ...
                if (buf.length >= 2 && buf[0] == -1 && buf[1] == -1) 
                {
                    ...
                    do 
                    {
                        ...
                        if (!this.glyphIds.contains(ogid)) 
                        {
                            ...
                            glyphIdsToAdd.add(new Integer(ogid));
                        }
                        off += 2;
            ...
            for (Integer glyphId : this.glyphIds) 
            {
                ...
                if (buf.length >= 2 && buf[0] == -1 && buf[1] == -1) 
                {
                    ...
                    do 
                    {
                        ...
                        if (!this.glyphIds.contains(ogid)) 
                        {
                            this.glyphIds.add(new Integer(ogid));
                        }
                        int ngid = this.getNewGlyphId(new Integer(ogid));
                        if (LOG.isDebugEnabled()) 
                        {
                            LOG.debug("mapped glyph  %d to %d in compound reference (flags=%04x)");
                        }
                        buf[off]   = (byte)(ngid >>> 8);
                        buf[off+1] = (byte)ngid;
                        off += 2;
    ...
    private byte[] buildCmapTable() throws IOException 
    {
        ...
        while(it.hasNext()) 
        {
            ...
            if (curChar.getKey().intValue() != prevChar.getKey().intValue()+1 ||
                    curGid - lastGid != curChar.getKey().intValue() - lastChar.getKey().intValue()) 
            {
                // Don't emit ranges, which map to the undef glyph, the
                // undef glyph is emitted a the very last segment.
                if (lastGid != 0) 
                {
                    startCode[nseg] = lastChar.getKey().intValue();
                    endCode[nseg] = prevChar.getKey().intValue();
                    idDelta[nseg] = lastGid - lastChar.getKey().intValue();
                    ++nseg;
                }
                // shorten ranges which start with undef by one.
                else if (!lastChar.getKey().equals(prevChar.getKey())) 
                {
                    startCode[nseg] = lastChar.getKey().intValue()+1;
                    endCode[nseg] = prevChar.getKey().intValue();
                    idDelta[nseg] = lastGid - lastChar.getKey().intValue();
                    ++nseg;
                }
        ...
        if (glyphNames == null) 
        {
            ...
            for (Integer glyphId : this.glyphIds) 
            {
                ...
                if (macId == null) 
                {
                    ...
                    if (idx == null) 
                    {
                        idx = new Integer(additionalNames.size());
    ...
    private byte[] buildHmtxTable() throws IOException 
    {
        ...
        try 
        {
            ...
            for (Integer glyphId : this.glyphIds) 
            {
                ...
                long off;
                if (glyphId.intValue() < h.getNumberOfHMetrics()) 
                {
                    off = glyphId.intValue() * 4;
                }
                else 
                {
                    off = h.getNumberOfHMetrics() * 4 + (glyphId.intValue() - h.getNumberOfHMetrics()) * 2;
                }
                ...
                int n = glyphId.intValue() < h.getNumberOfHMetrics() ? 4 : 2;
