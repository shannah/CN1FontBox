...
import ca.weblite.codename1.lang.Rectangle2D;
...
public class AFMFormatter
{
            ...
            throws IOException
    {
        ...
        for (CharMetric metric : metrics)
        {
            ...
            output.print("B " + (int) metric.bounds.getX() + " "
                    ...
                    + (int) (metric.bounds.getX()+metric.bounds.getWidth()) + " "
                    + (int) (metric.bounds.getY() + metric.bounds.getHeight())+" ;");
            output.println();
        }
    ...
    private static Rectangle2D getBounds(List<CharMetric> metrics)
    {
        ...
        for(CharMetric metric : metrics)
        {
            if(bounds == null)
            {
                bounds = new Rectangle2D(metric.bounds);
               
                
            }
            else
            {
                bounds = bounds.union(metric.bounds);
